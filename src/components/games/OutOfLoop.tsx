...
import { isBoolean, isOutOfLoopRound, isPlainRecord, isShortString, isStringList, isSafeInteger, useGameSessionState } from "../../lib/useGameSessionState";
...
export function OutOfLoop({ locale }: OutOfLoopProps) {
  ...
  const [round, setRound] = useGameSessionState<GameRound | null>("out-of-loop", locale, "round", null, (value): value is GameRound | null => value === null || isOutOfLoopRound(value));
  ...
  const [votes, setVotes] = useGameSessionState<Record<string, string>>("out-of-loop", locale, "votes", {}, (value): value is Record<string, string> => isPlainRecord(value) && Object.keys(value).length <= 12 && Object.values(value).every(isShortString));
    ...
    <>
        ...
        {screen === "setup" && (
          <SetupShell title={t.who} hint={t.whoHint}>
            ...
            <button data-action="primary" className="primary" disabled={!setupValidation.valid} onClick={startRound}>{t.assign} <Shuffle size={20} /></button>
          </SetupShell>
        )}
